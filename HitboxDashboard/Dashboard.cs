...
namespace HitboxDashboard
{
    public partial class HitboxForm
    {
        ...
        {
            if (IsHandleCreated && isLogged)
            {
                BeginInvoke((MethodInvoker) delegate { GetTitle(); });
            }
        }
        ...
        private void hiddenTimer_Tick(object sender, EventArgs e)
        {
            if (IsHandleCreated && isLogged)
            {
                BeginInvoke((MethodInvoker)delegate { GetHidden(); });
            }
        }
        ...
        private void gameTimer_Tick(object sender, EventArgs e)
        {
            if (IsHandleCreated && isLogged)
            {
                BeginInvoke((MethodInvoker)delegate { GetGame(); });
            }
        }
        #endregion
        ...
        {
            Media livestream2 = new Media {media_id = UserId, media_status = title, media_user_name = Channel, 
                media_category_id = GetGameId(gameComboBox.Text), media_recording = GetRecordings().ToString(), media_hidden = GetHiddenBoolean()};
            ...
            API.Put(String.Format("{0}/media/live/{1}.json?showHidden=true&authToken={2}&nocache=true", API.BaseApi, Channel, AuthToken), json);
            streamTitle.Multiline = false;
            streamTitle.Multiline = true;
            groupBox1.Focus();
        }
        ...
        {
            // category_name
            if (gameComboBox.Focused) return;
            if (gameComboBox.DroppedDown) return;
        ...
        private void PostGame(String gameName)
        {
            ...
            #endregion
            Media livestream2 = new Media {media_id = UserId, media_status = streamTitle.Text, media_user_name = Channel, 
                media_category_id = GetGameId(gameName), media_recording = GetRecordings().ToString(), media_hidden = GetHiddenBoolean()};
            List<Media> livestream = new List<Media> { livestream2 };
        ...
        {
            var p = API.Get(String.Format("{0}/media/live/{1}.json?showHidden=true&authToken={2}&nocache=true",API.BaseApi, Channel, AuthToken));
            var ll = JObject.Parse(p.ToString()).GetValue("livestream").First;
        ...
        {
            Media livestream2 = new Media{media_id = UserId, media_status = streamTitle.Text, media_user_name = Channel,
                media_category_id = GetGameId(gameComboBox.Text), media_recording = GetRecordings().ToString(), media_hidden = hidden
            };
            List<Media> livestream = new List<Media> { livestream2 };
        ...
        {
            RadioButton checkedButton = visibleBox.Controls.OfType<RadioButton>()
                                      .FirstOrDefault(r => r.Checked);
            return checkedButton.Text.Equals("Visible") ? false : true;
        }
        ...
        private void GetEditors(String username)
        {
            ...
            editorDropDown.Visible = true;
            editorDropDown.DroppedDown = true;
